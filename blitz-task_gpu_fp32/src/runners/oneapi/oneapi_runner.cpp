 ...
 * @brief Level Zero / oneAPI fp32 dense-FMA runner.
 *
...
RunResult run_gpu_fp32_oneapi(const gpgpu::Setup& setup) {
  ...
  ze_module_handle_t module =
      create_module_with_log(ze, ctx, dev, k_fp32_fma_spv_bytes, k_fp32_fma_spv_bytes_len, "", log, rc);
  if (!module) {
  ...
  ze.zeKernelSetGroupSize(kernel, fp32::kBlock, 1, 1);
