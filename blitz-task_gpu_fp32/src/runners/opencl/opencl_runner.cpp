 ...
 * @brief OpenCL fp32 dense-FMA runner.
 *
...
RunResult run_gpu_fp32_opencl(const gpgpu::Setup& setup) {
  ...
  cl_kernel kernel = cl.clCreateKernel(program, "fp32_fma", &err);
  if (!kernel) {
  ...
  cl.clEnqueueReadBuffer(queue, buf, CL_TRUE, 0, fp32::kPreflightThreads * sizeof(float), host.data(), 0, nullptr,
                         nullptr);
