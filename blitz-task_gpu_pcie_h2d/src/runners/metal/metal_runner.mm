 ...
 * @brief Metal host-to-device transfer runner (macOS only).
 *
 * A storageModeShared source (filled once) is blit-copied into a
 * storageModePrivate destination with an MTLBlitCommandEncoder, timed with
 * MTLCommandBuffer GPU{Start,End}Time. Apple GPUs share DRAM with the CPU, so
 * this measures a DRAM↔DRAM blit — not a discrete interconnect; the path says so.
 * Verified once by blitting the destination back into a shared readback buffer.
 */
...
RunResult run_gpu_h2d_metal(const gpgpu::Setup& setup) {
    RunResult r;
    r.path       = "shared->private blit (unified mem)";
    r.score_unit = "GB/s";
    ...
    @autoreleasepool {
        ...
        id<MTLBuffer> src = [device newBufferWithLength:S options:MTLResourceStorageModeShared];
        ...
        {
            std::uint32_t* p = static_cast<std::uint32_t*>([src contents]);
        ...
        // --- verify once: blit dst → shared readback ---
        id<MTLBuffer> readback = [device newBufferWithLength:S options:MTLResourceStorageModeShared];
        {
            id<MTLCommandBuffer> cmd = [queue commandBuffer];
            id<MTLBlitCommandEncoder> enc = [cmd blitCommandEncoder];
            [enc copyFromBuffer:dst sourceOffset:0 toBuffer:readback destinationOffset:0 size:S];
            [enc endEncoding];
            [cmd commit];
            [cmd waitUntilCompleted];
        }
        std::vector<std::uint32_t> host(count);
        std::memcpy(host.data(), [readback contents], S);
