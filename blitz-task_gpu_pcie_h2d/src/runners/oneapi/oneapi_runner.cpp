 ...
 * @brief Level Zero / oneAPI host-to-device transfer runner — SDK-required variant.
 *
...
RunResult run_gpu_h2d_oneapi(const gpgpu::Setup& setup) {
  ...
  void* h_src = nullptr;
  zeMemAllocHost(ctx, &had, S, sizeof(std::uint32_t), &h_src);

  ze_device_mem_alloc_desc_t mad{};
  mad.stype = ZE_STRUCTURE_TYPE_DEVICE_MEM_ALLOC_DESC;
  void* d_dst = nullptr;
  zeMemAllocDevice(ctx, &mad, S, sizeof(std::uint32_t), dev, &d_dst);

  if (!h_src || !d_dst) {
    if (h_src) zeMemFree(ctx, h_src);
  ...
  auto time_copies = [&](int reps) -> double {
    ...
    for (int i = 0; i < reps; ++i) {
      ...
      zeCommandListAppendMemoryCopy(list, d_dst, h_src, S, sig, 0, nullptr);
  ...
  zeCommandListReset(list);
  zeCommandListAppendMemoryCopy(list, host.data(), d_dst, S, nullptr, 0, nullptr);
  zeCommandListClose(list);
  zeCommandQueueExecuteCommandLists(cqueue, 1, &list, nullptr);
  zeCommandQueueSynchronize(cqueue, UINT64_MAX);
