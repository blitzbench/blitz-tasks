 ...
 * @brief Vulkan host-to-device transfer runner.
 *
...
RunResult run_gpu_h2d_vulkan(const gpgpu::Setup& setup) {
  ...
  auto teardown = [&](VkQueryPool qp, VkCommandPool cp) {
    ...
    destroy_buffer(vkd, dev, readback);
  ...
  auto time_copies = [&](int reps) -> double {
    ...
    for (int i = 0; i < reps; ++i) {
      vkd.vkCmdCopyBuffer(cb, staging.buf, dst.buf, 1, &region);
      if (i + 1 < reps)
  ...
  vkd.vkBeginCommandBuffer(cb, &bi);
  VkBufferCopy region{0, 0, bytes};
  vkd.vkCmdCopyBuffer(cb, dst.buf, readback.buf, 1, &region);
  vkd.vkEndCommandBuffer(cb);
  submit_wait();
  ...
  {
    void* m = nullptr;
    vkd.vkMapMemory(dev, readback.mem, 0, bytes, 0, &m);
    std::memcpy(host.data(), m, S);
    vkd.vkUnmapMemory(dev, readback.mem);
  }
