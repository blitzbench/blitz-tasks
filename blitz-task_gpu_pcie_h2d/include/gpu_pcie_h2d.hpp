...
/**
 * @class GpuPcieH2d
 * @brief Measures host-to-device (PCIe) transfer bandwidth.
 *
 * copies from pinned host memory to the GPU over the PCIe link, so the result
 * reflects sustained upload bandwidth. Reported in GB/s, higher is better.
 */
class CPP_TASK_DEMO_EXPORT GpuPcieH2d : public blitz::Task {
 public:
  GpuPcieH2d();
  ~GpuPcieH2d() override;
...
extern "C" {
CPP_TASK_DEMO_EXPORT BlitzTask* gpu_pcie_h2d_new(void);
}
