...
RunResult run_gpu_fp16_oneapi(const gpgpu::Setup& setup) {
  ...
  ze_module_handle_t module =
      create_module_with_log(ze, ctx, dev, k_fp16_packed_spv_bytes, k_fp16_packed_spv_bytes_len, "", log, rc);
  if (!module) {
  ...
  ze.zeKernelSetGroupSize(kernel, fp16::kBlock, 1, 1);
  ...
  void* d_out = nullptr;
  ze.zeMemAllocDevice(ctx, &mad, bytes, alignof(float), dev, &d_out);
  ...
  auto read_back = [&](std::uint32_t count, std::vector<float>& host) {
    ze.zeCommandListReset(list);
    ze.zeCommandListAppendMemoryCopy(list, host.data(), d_out, count * sizeof(float), nullptr, 0, nullptr);
    ze.zeCommandListClose(list);
    ze.zeCommandQueueExecuteCommandLists(queue, 1, &list, nullptr);
    ze.zeCommandQueueSynchronize(queue, UINT64_MAX);
  };
