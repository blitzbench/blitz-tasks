...
/**
 * @class GpuFp16
 * @brief Measures half-precision (fp16) compute throughput on the GPU.
 *
 * runs a dense fused-multiply-add kernel (tensor / cooperative-matrix where
 * available, packed fp16 otherwise) with data kept on-chip, so the result
 * reflects raw fp16 throughput rather than  memory transfer. Reported in GFLOPS,
 * higher is better.
 */
class CPP_TASK_DEMO_EXPORT GpuFp16 : public blitz::Task {
 public:
  GpuFp16();
  ~GpuFp16() override;
...
extern "C" {
CPP_TASK_DEMO_EXPORT BlitzTask* gpu_fp16_new(void);
}
