 ...
 * @brief Level Zero / oneAPI int8 throughput runner.
 *
...
RunResult run_gpu_int8_oneapi(const gpgpu::Setup& setup) {
  ...
  ze_module_handle_t module =
      create_module_with_log(ze, ctx, dev, k_int8_packed_spv_bytes, k_int8_packed_spv_bytes_len, "", log, rc);
  if (!module) {
  ...
  ze.zeKernelSetGroupSize(kernel, i8::kBlock, 1, 1);
  ...
  void* d_out = nullptr;
  ze.zeMemAllocDevice(ctx, &mad, bytes, alignof(std::uint32_t), dev, &d_out);
  ...
  auto read_back = [&](std::uint32_t count, std::vector<std::uint32_t>& host) {
    ze.zeCommandListReset(list);
    ze.zeCommandListAppendMemoryCopy(list, host.data(), d_out, count * sizeof(std::uint32_t), nullptr, 0, nullptr);
    ze.zeCommandListClose(list);
    ze.zeCommandQueueExecuteCommandLists(queue, 1, &list, nullptr);
    ze.zeCommandQueueSynchronize(queue, UINT64_MAX);
  };
