...
/**
 * @class GpuVramCopy
 * @brief Measures on-device VRAM copy bandwidth.
 *
 * streams a large buffer device-to-device (read plus write) entirely in VRAM, so
 * the result reflects sustained on-board memory bandwidth. Reported in GB/s,
 * higher is better.
 */
class CPP_TASK_DEMO_EXPORT GpuVramCopy : public blitz::Task {
 public:
  GpuVramCopy();
  ~GpuVramCopy() override;
...
extern "C" {
CPP_TASK_DEMO_EXPORT BlitzTask* gpu_vram_copy_new(void);
}
