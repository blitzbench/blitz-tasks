...
#if defined(GPU_BIDIR_HAVE_CUDA)
#include "runners/cuda/cuda_runner.hpp"
#endif
#if defined(GPU_BIDIR_HAVE_ROCM)
#include "runners/rocm/rocm_runner.hpp"
#endif
#if defined(GPU_BIDIR_HAVE_ONEAPI)
#include "runners/oneapi/oneapi_runner.hpp"
#endif
#if defined(GPU_BIDIR_HAVE_OPENCL)
#include "runners/opencl/opencl_runner.hpp"
#endif
#if defined(GPU_BIDIR_HAVE_VULKAN)
#include "runners/vulkan/vulkan_runner.hpp"
#endif
#if defined(GPU_BIDIR_HAVE_METAL)
#include "runners/metal/metal_runner.hpp"
#endif

extern "C" const char* GPU_PCIE_BIDIR_INFO_JSON;
 ...
 *        guarded by the GPU_BIDIR_HAVE_<BACKEND> defines the linked runner libs export.
 *
 * @param setup
 * @return
 */
...
#if defined(GPU_BIDIR_HAVE_CUDA)
      return bench::run_gpu_bidir_cuda(setup);
#else
      return missing("CUDA");
#endif
    case gpgpu::BackendId::ROCm:
#if defined(GPU_BIDIR_HAVE_ROCM)
      return bench::run_gpu_bidir_rocm(setup);
#else
      return missing("ROCm");
#endif
    case gpgpu::BackendId::OneAPI:
#if defined(GPU_BIDIR_HAVE_ONEAPI)
      return bench::run_gpu_bidir_oneapi(setup);
#else
      return missing("OneAPI");
#endif
    case gpgpu::BackendId::OpenCL:
#if defined(GPU_BIDIR_HAVE_OPENCL)
      return bench::run_gpu_bidir_opencl(setup);
#else
      return missing("OpenCL");
#endif
    case gpgpu::BackendId::Vulkan:
#if defined(GPU_BIDIR_HAVE_VULKAN)
      return bench::run_gpu_bidir_vulkan(setup);
#else
      return missing("Vulkan");
#endif
    case gpgpu::BackendId::Metal:
#if defined(GPU_BIDIR_HAVE_METAL)
      return bench::run_gpu_bidir_metal(setup);
#else
...
GpuPcieBidir::GpuPcieBidir() : timeout_ms_(DEFAULT_BUDGET_MS) {}

GpuPcieBidir::~GpuPcieBidir() = default;

std::string_view GpuPcieBidir::info_json() const noexcept { return GPU_PCIE_BIDIR_INFO_JSON; }

blitz::Result GpuPcieBidir::configure(const blitz::DataConfig& cfg) {
...
blitz::Result GpuPcieBidir::set_timeout(const std::uint64_t timeout_ms) {
  timeout_ms_ = timeout_ms;
  return BLITZ_OK;
}

blitz::Result GpuPcieBidir::setSetup(const gpgpu::Setup& setup) {
...
std::vector<bench::gpu::ProbeResult> GpuPcieBidir::probeSetups(
    const std::vector<gpgpu::Setup>& setups) const {
  return bench::gpu::probe_setups(setups, dispatch);
}

blitz::Result GpuPcieBidir::run(const blitz::Callbacks& cb) {
  if (!setup_) {
    if (cb.on_error) {
...
extern "C" ::BlitzTask* gpu_pcie_bidir_new(void) {
  return blitz::make_c_task(std::make_unique<gpu_pcie_bidir::GpuPcieBidir>());
}
