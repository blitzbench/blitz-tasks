/**
 * @file cuda_runner.cu
 * @brief CUDA fp32 GEMM runner — cuBLASLt first, with a tiled kernel fallback.
 *
 * Runtime selection:
 *   * cuBLASLt resolved -> cublasLtMatmul, CUDA_R_32F operands and result with
 *     CUBLAS_COMPUTE_32F_FAST_TF32   path="blas(cuBLASLt <version>, tf32)"
 *   * otherwise         -> 16x16 shared-memory tile  path="tiled(shared 16x16 fp32)"
 *
 * The compute type is set explicitly rather than left to the library: cuBLAS has changed
 * whether TF32 is used by default between versions, and this task exists to report the
 * fastest fp32-in/fp32-out path the vendor offers, which on tensor hardware is TF32.
 * Operand values are chosen so they stay exact under TF32's 10-bit mantissa, so enabling
 * it does not weaken the verification.
 *
...
bool build_context(const gpgpu::Setup& setup, std::uint32_t n, std::uint32_t seed, CudaGemmContext& out,
    ...
    if (cudaMalloc(&out.a, elems * sizeof(float)) != cudaSuccess ||
        cudaMalloc(&out.b, elems * sizeof(float)) != cudaSuccess ||
    ...
    if (api.resolved()) {
        ...
        const bool built =
            ...
            api.desc_create(&out.desc, CUBLAS_COMPUTE_32F_FAST_TF32, CUDA_R_32F) == CUBLAS_STATUS_SUCCESS &&
            api.layout_create(&out.layout_a, CUDA_R_32F, n, n, n) == CUBLAS_STATUS_SUCCESS &&
            api.layout_create(&out.layout_b, CUDA_R_32F, n, n, n) == CUBLAS_STATUS_SUCCESS &&
...
RunResult run_gpu_matmul_fp32_cuda(const gpgpu::Setup& setup, gemm::ContextPtr& ctx,
    ...
    const CublasLtApi& api = cublaslt_api();
    r.path = cu->use_blas
                 ? ("blas(cuBLASLt " + (api.version.empty() ? std::string("unknown") : api.version) + ", tf32)")
    ...
    r.info = {{"gemm_n", std::to_string(cu->n)},
              ...
              {"math_mode", cu->use_blas ? "fp32 in/out, tf32 compute" : "fp32 throughout"}};
    return r;
}
