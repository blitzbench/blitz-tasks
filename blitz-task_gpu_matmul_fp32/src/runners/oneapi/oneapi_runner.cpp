...
bool build_context(const gpgpu::vendor::LevelZeroFns& ze, const gpgpu::Setup& setup, std::uint32_t n,
    ...
    if (ze.zeMemAllocDevice(out.ctx, &mdesc, elems * 4, 64, out.device, &out.a) != ZE_RESULT_SUCCESS ||
        ze.zeMemAllocDevice(out.ctx, &mdesc, elems * 4, 64, out.device, &out.b) != ZE_RESULT_SUCCESS ||
