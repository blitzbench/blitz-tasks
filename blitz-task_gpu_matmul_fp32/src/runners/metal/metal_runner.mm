 ...
 * @brief Metal fp32 GEMM runner — MetalPerformanceShaders matrix multiply.
 *
 ...
 * Operands and result are fp32, matching every other backend of this task.
 *
...
kernel void gemm_fill(device float* a [[buffer(0)]],
    ...
    a[idx] = gemm_operand_value(gid.y, gid.x, 0u, seed);
    b[idx] = gemm_operand_value(gid.y, gid.x, 1u, seed);
}
)MSL";
...
bool build_context(const gpgpu::Setup& setup, std::uint32_t n, std::uint32_t seed, MetalGemmContext& out,
    ...
    out.a = [out.device newBufferWithLength:elems * sizeof(float) options:options];
    out.b = [out.device newBufferWithLength:elems * sizeof(float) options:options];
