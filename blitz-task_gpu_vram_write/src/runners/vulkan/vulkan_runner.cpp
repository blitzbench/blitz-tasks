...
RunResult run_gpu_vram_write_vulkan(const gpgpu::Setup& setup) {
  ...
  bool alloc_ok = create_buffer(vkd, dev, mp, S, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, buf) &&
                  create_buffer(vkd, dev, mp, static_cast<VkDeviceSize>(w) * 4, VK_BUFFER_USAGE_TRANSFER_DST_BIT,
  ...
  auto check_window = [&](std::uint32_t start) {
    ...
    vkd.vkCmdCopyBuffer(cb, buf.buf, staging.buf, 1, &region);
    vkd.vkEndCommandBuffer(cb);
    VkSubmitInfo si{VK_STRUCTURE_TYPE_SUBMIT_INFO};
    si.commandBufferCount = 1;
    si.pCommandBuffers = &cb;
