...
RunResult run_gpu_vram_write_oneapi(const gpgpu::Setup& setup) {
  ...
  ze_module_handle_t module =
      create_module_with_log(ze, ctx, dev, k_vram_write_spv_bytes, k_vram_write_spv_bytes_len, "", log, rc);
  if (!module) {
  ...
  void* d_buf = nullptr;
  ze.zeMemAllocDevice(ctx, &mad, S, 16, dev, &d_buf);
  ze.zeKernelSetArgumentValue(kernel, 0, sizeof(void*), &d_buf);
  ...
  auto read_window = [&](std::uint32_t start, std::vector<std::uint32_t>& host) {
    ze.zeCommandListReset(list);
    ze.zeCommandListAppendMemoryCopy(list, host.data(),
