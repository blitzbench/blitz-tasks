...
#if defined(GPU_MATMUL_FP16_HAVE_CUDA)
#include "runners/cuda/cuda_runner.hpp"
#endif
#if defined(GPU_MATMUL_FP16_HAVE_ROCM)
#include "runners/rocm/rocm_runner.hpp"
#endif
#if defined(GPU_MATMUL_FP16_HAVE_ONEAPI)
#include "runners/oneapi/oneapi_runner.hpp"
#endif
#if defined(GPU_MATMUL_FP16_HAVE_OPENCL)
#include "runners/opencl/opencl_runner.hpp"
#endif
#if defined(GPU_MATMUL_FP16_HAVE_VULKAN)
#include "runners/vulkan/vulkan_runner.hpp"
#endif
#if defined(GPU_MATMUL_FP16_HAVE_METAL)
#include "runners/metal/metal_runner.hpp"
#endif

extern "C" const char* GPU_MATMUL_FP16_INFO_JSON;
 ...
 *        GPU_MATMUL_FP16_HAVE_<BACKEND> defines the linked runner libs export.
 *
 * @param setup
 * @param ctx
 * @param params
...
#if defined(GPU_MATMUL_FP16_HAVE_CUDA)
      return bench::run_gpu_matmul_fp16_cuda(setup, ctx, params);
#else
      return missing("CUDA");
#endif
    case gpgpu::BackendId::ROCm:
#if defined(GPU_MATMUL_FP16_HAVE_ROCM)
      return bench::run_gpu_matmul_fp16_rocm(setup, ctx, params);
#else
      return missing("ROCm");
#endif
    case gpgpu::BackendId::OneAPI:
#if defined(GPU_MATMUL_FP16_HAVE_ONEAPI)
      return bench::run_gpu_matmul_fp16_oneapi(setup, ctx, params);
#else
      return missing("OneAPI");
#endif
    case gpgpu::BackendId::OpenCL:
#if defined(GPU_MATMUL_FP16_HAVE_OPENCL)
      return bench::run_gpu_matmul_fp16_opencl(setup, ctx, params);
#else
      return missing("OpenCL");
#endif
    case gpgpu::BackendId::Vulkan:
#if defined(GPU_MATMUL_FP16_HAVE_VULKAN)
      return bench::run_gpu_matmul_fp16_vulkan(setup, ctx, params);
#else
      return missing("Vulkan");
#endif
    case gpgpu::BackendId::Metal:
#if defined(GPU_MATMUL_FP16_HAVE_METAL)
      return bench::run_gpu_matmul_fp16_metal(setup, ctx, params);
#else
...
GpuMatmulFp16::GpuMatmulFp16() : timeout_ms_(DEFAULT_BUDGET_MS) {}

GpuMatmulFp16::~GpuMatmulFp16() = default;

std::string_view GpuMatmulFp16::info_json() const noexcept { return GPU_MATMUL_FP16_INFO_JSON; }

blitz::Result GpuMatmulFp16::configure(const blitz::DataConfig& cfg) {
...
blitz::Result GpuMatmulFp16::set_timeout(const std::uint64_t timeout_ms) {
  timeout_ms_ = timeout_ms;
  return BLITZ_OK;
}

blitz::Result GpuMatmulFp16::setSetup(const gpgpu::Setup& setup) {
...
std::vector<bench::gpu::ProbeResult> GpuMatmulFp16::probeSetups(const std::vector<gpgpu::Setup>& setups) const {
  ...
  params.cap_bytes = bench::gemm::smallest_rung_cap(bench::gemm::Precision::Fp16);
...
blitz::Result GpuMatmulFp16::run(const blitz::Callbacks& cb) {
  if (!setup_) {
    if (cb.on_error) {
...
extern "C" ::BlitzTask* gpu_matmul_fp16_new(void) {
  return blitz::make_c_task(std::make_unique<gpu_matmul_fp16::GpuMatmulFp16>());
}
