 ...
 * @brief OpenCL fp16 GEMM runner — shared-memory tiled multiply.
 *
 * OpenCL has no vendor BLAS this task can call, so it always reports the tiled path:
 *
 *   "tiled(local 16x16 f16->f32)"
 *
 * Operands are held as half and widened on load with vload_half, which is core OpenCL
 * and so needs no cl_khr_fp16: the inputs carry fp16 precision and the accumulator is
 * fp32, matching every other backend of this task. gemm_operand_value mirrors
 * bench::gemm::operand_value, and the fill kernel writes A and B on the device so no
 * operand crosses the bus.
 *
...
__kernel void gemm_fill(__global half* a, __global half* b, uint n, uint seed) {
    const uint col = get_global_id(0);
    const uint row = get_global_id(1);
    if (row >= n || col >= n) return;
    const uint idx = row * n + col;
    vstore_half(gemm_operand_value(row, col, 0u, seed), idx, a);
    vstore_half(gemm_operand_value(row, col, 1u, seed), idx, b);
}

__kernel void gemm_tiled(__global const half* a, __global const half* b, __global float* c, uint n) {
    ...
    for (uint t = 0; t < n; t += 16u) {
        sa[lr][lc] = vload_half(row * n + (t + lc), a);
        sb[lr][lc] = vload_half((t + lr) * n + col, b);
...
bool build_context(const gpgpu::vendor::OpenClFns& cl, const gpgpu::Setup& setup, std::uint32_t n,
  ...
  out.a = cl.clCreateBuffer(out.ctx, CL_MEM_READ_WRITE, elems * 2, nullptr, &err);
  out.b = cl.clCreateBuffer(out.ctx, CL_MEM_READ_WRITE, elems * 2, nullptr, &err);
...
RunResult run_gpu_matmul_fp16_opencl(const gpgpu::Setup& setup, gemm::ContextPtr& ctx,
  ...
  r.path = "tiled(local 16x16 f16->f32)";
