...
/**
 * @brief OpenCL runner for gpu_matmul_fp16. Always compiled; the OpenCL runtime is bound
 *        at run time through gpgpu::vendor, so a machine without it only loses this
 *        backend. The lib target sets GPU_MATMUL_FP16_HAVE_OPENCL which gates inclusion from
 *        the task's dispatch().
 *
 * @param setup
