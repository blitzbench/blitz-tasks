...
/**
 * @brief oneAPI (Level Zero) runner for gpu_matmul_fp16. Built only when the Level Zero
 *        loader and a GLSL compiler were detected at configure time; the lib target sets
 *        GPU_MATMUL_FP16_HAVE_ONEAPI which gates inclusion from the task's dispatch().
 *
 * @param setup
 ...
 * @return
 */
RunResult run_gpu_matmul_fp16_oneapi(const gpgpu::Setup& setup, gemm::ContextPtr& ctx,
