...
/**
 * @class GpuVramRead
 * @brief Measures on-device VRAM streaming read bandwidth.
 *
 * streams loads from a large VRAM buffer, so the result reflects sustained
 * on-board read bandwidth. Reported in GB/s, higher is better.
 */
class CPP_TASK_DEMO_EXPORT GpuVramRead : public blitz::Task {
 public:
  GpuVramRead();
  ~GpuVramRead() override;
...
extern "C" {
CPP_TASK_DEMO_EXPORT BlitzTask* gpu_vram_read_new(void);
}
