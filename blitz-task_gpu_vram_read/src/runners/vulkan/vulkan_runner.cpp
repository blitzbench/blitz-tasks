...
RunResult run_gpu_vram_read_vulkan(const gpgpu::Setup& setup) {
  ...
  bool alloc_ok =
      create_buffer(vkd, dev, mp, S, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                    VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, in_buf) &&
      create_buffer(vkd, dev, mp, out_bytes, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                    VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, out_buf) &&
      create_buffer(vkd, dev, mp, S, VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
  ...
  vkd.vkUpdateDescriptorSets(dev, 2, wr, 0, nullptr);
  ...
  auto cleanup = [&]() {
    ...
    destroy_buffer(vkd, dev, in_buf);
    destroy_buffer(vkd, dev, out_buf);
  ...
  {
    void* mapped = nullptr;
    vkd.vkMapMemory(dev, staging.mem, 0, S, 0, &mapped);
    ...
    vkd.vkCmdCopyBuffer(cb, staging.buf, in_buf.buf, 1, &region);
    submit_wait();
  }
  ...
  std::string verr;
  {
    ...
    vkd.vkCmdCopyBuffer(cb, out_buf.buf, staging.buf, 1, &region);
    submit_wait();
    void* mapped = nullptr;
    vkd.vkMapMemory(dev, staging.mem, 0, out_bytes, 0, &mapped);
