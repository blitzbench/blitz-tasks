 ...
 * loop is bracketed by timestamp queries (scaled by limits.timestampPeriod; the
 * queue's timestampValidBits are checked). On a UMA / software device with no
 * DEVICE_LOCAL-only heap the copy still runs and the path notes it. The mapped
 * destination is sample-checked once. Exact mirror of the h2d Vulkan runner.
 */
...
RunResult run_gpu_d2h_vulkan(const gpgpu::Setup& setup) {
  RunResult r;
  ...
  // --- verify once: the received (mapped) destination ---
