...
#if defined(GPU_D2H_HAVE_CUDA)
#include "runners/cuda/cuda_runner.hpp"
#endif
#if defined(GPU_D2H_HAVE_ROCM)
#include "runners/rocm/rocm_runner.hpp"
#endif
#if defined(GPU_D2H_HAVE_ONEAPI)
#include "runners/oneapi/oneapi_runner.hpp"
#endif
#if defined(GPU_D2H_HAVE_OPENCL)
#include "runners/opencl/opencl_runner.hpp"
#endif
#if defined(GPU_D2H_HAVE_VULKAN)
#include "runners/vulkan/vulkan_runner.hpp"
#endif
#if defined(GPU_D2H_HAVE_METAL)
#include "runners/metal/metal_runner.hpp"
#endif

extern "C" const char* GPU_PCIE_D2H_INFO_JSON;

namespace gpu_pcie_d2h {
 ...
 *        guarded by the GPU_D2H_HAVE_<BACKEND> defines the linked runner libs export.
 *
 * @param setup
 * @return
 */
...
#if defined(GPU_D2H_HAVE_CUDA)
      return bench::run_gpu_d2h_cuda(setup);
#else
      return missing("CUDA");
#endif
    case gpgpu::BackendId::ROCm:
#if defined(GPU_D2H_HAVE_ROCM)
      return bench::run_gpu_d2h_rocm(setup);
#else
      return missing("ROCm");
#endif
    case gpgpu::BackendId::OneAPI:
#if defined(GPU_D2H_HAVE_ONEAPI)
      return bench::run_gpu_d2h_oneapi(setup);
#else
      return missing("OneAPI");
#endif
    case gpgpu::BackendId::OpenCL:
#if defined(GPU_D2H_HAVE_OPENCL)
      return bench::run_gpu_d2h_opencl(setup);
#else
      return missing("OpenCL");
#endif
    case gpgpu::BackendId::Vulkan:
#if defined(GPU_D2H_HAVE_VULKAN)
      return bench::run_gpu_d2h_vulkan(setup);
#else
      return missing("Vulkan");
#endif
    case gpgpu::BackendId::Metal:
#if defined(GPU_D2H_HAVE_METAL)
      return bench::run_gpu_d2h_metal(setup);
#else
...
GpuPcieD2h::GpuPcieD2h() : timeout_ms_(DEFAULT_BUDGET_MS) {}

GpuPcieD2h::~GpuPcieD2h() = default;

std::string_view GpuPcieD2h::info_json() const noexcept { return GPU_PCIE_D2H_INFO_JSON; }

blitz::Result GpuPcieD2h::configure(const blitz::DataConfig& cfg) {
...
blitz::Result GpuPcieD2h::set_timeout(const std::uint64_t timeout_ms) {
  timeout_ms_ = timeout_ms;
  return BLITZ_OK;
}

blitz::Result GpuPcieD2h::setSetup(const gpgpu::Setup& setup) {
...
std::vector<bench::gpu::ProbeResult> GpuPcieD2h::probeSetups(
    const std::vector<gpgpu::Setup>& setups) const {
  return bench::gpu::probe_setups(setups, dispatch);
}

blitz::Result GpuPcieD2h::run(const blitz::Callbacks& cb) {
  if (!setup_) {
    if (cb.on_error) {
...
extern "C" ::BlitzTask* gpu_pcie_d2h_new(void) {
  return blitz::make_c_task(std::make_unique<gpu_pcie_d2h::GpuPcieD2h>());
}
