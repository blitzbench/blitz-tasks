...
/**
 * @class GpuPcieD2h
 * @brief Measures device-to-host (PCIe) transfer bandwidth.
 *
 * copies from the GPU to pinned host memory over the PCIe link, so the result
 * reflects sustained download bandwidth. Reported in GB/s, higher is better.
 */
class CPP_TASK_DEMO_EXPORT GpuPcieD2h : public blitz::Task {
 public:
  GpuPcieD2h();
  ~GpuPcieD2h() override;
...
extern "C" {
CPP_TASK_DEMO_EXPORT BlitzTask* gpu_pcie_d2h_new(void);
}
