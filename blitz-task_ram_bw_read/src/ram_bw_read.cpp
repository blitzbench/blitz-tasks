...
extern "C" const char* RAM_BW_READ_INFO_JSON;
...
bench::Dispatched<BwKernel> kernel_table() {
    ...
    t.scalar = &read_scalar;
#if defined(__x86_64__) || defined(__i386__)
    t.sse2 = &read_sse2;
    t.avx = &read_avx;
    t.avx512 = &read_avx512;
...
RamBwRead::RamBwRead() : timeout_ms_(DEFAULT_BUDGET_MS) {}

RamBwRead::~RamBwRead() = default;

std::string_view RamBwRead::info_json() const noexcept {
    return RAM_BW_READ_INFO_JSON;
}

blitz::Result RamBwRead::configure(const blitz::DataConfig& cfg) {
    data_size_bytes_ = cfg.data_size_bytes;
    seed_ = cfg.seed;
    return BLITZ_OK;
}

blitz::Result RamBwRead::set_timeout(std::uint64_t timeout_ms) {
    timeout_ms_ = timeout_ms;
    return BLITZ_OK;
}

blitz::Result RamBwRead::run(const blitz::Callbacks& cb) {
    ...
    // Resolved before the clock starts, as cpu_dispatch.h requires.
    bench::SimdTier tier = bench::SimdTier::Scalar;
    const BwKernel fn = kernel_table().best(&tier);
    if (!fn) return fail(BLITZ_ERR_UNSUPPORTED, "no read kernel for this architecture");

    // TASK.json: "across all cores". Single-core bandwidth is capped by a
    // core's outstanding-miss buffers, not by the DIMMs, so only every core at
    // once approaches the platform maximum.
    ...
    bench::Buffer buf(ws);
    if (!buf) return fail(BLITZ_ERR_RESOURCE, "could not allocate the read buffer");
    // Mandatory: without this the buffer is zero-page mappings and the kernel
    // would measure L1 rather than DRAM, silently.
    ...
    const double bytes_per_sec = bench::run_timed_parallel(
        ...
        [&](unsigned i) {
            ...
            const bench::Slice s = bench::slice_for(ws, threads, i);
            const char* src = base + s.offset;
            const std::size_t n = s.size;
            return [fn, src, n] { return fn(src, n); };
        },
...
extern "C" ::BlitzTask* ram_bw_read_new(void) {
    return blitz::make_c_task(std::make_unique<ram_bw_read::RamBwRead>());
}
