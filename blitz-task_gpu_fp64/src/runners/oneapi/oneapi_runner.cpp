 ...
 * @brief Level Zero / oneAPI fp64 dense-FMA runner.
 *
...
RunResult run_gpu_fp64_oneapi(const gpgpu::Setup& setup) {
  ...
  ze.zeDeviceGetModuleProperties(dev, &mod_props);
  ...
  ze_module_handle_t module =
      create_module_with_log(ze, ctx, dev, k_fp64_fma_spv_bytes, k_fp64_fma_spv_bytes_len, "", log, rc);
  if (!module) {
  ...
  ze.zeKernelSetGroupSize(kernel, fp64::kBlock, 1, 1);
  ...
  void* d_out = nullptr;
  ze.zeMemAllocDevice(ctx, &mad, bytes, alignof(double), dev, &d_out);
  ze.zeKernelSetArgumentValue(kernel, 0, sizeof(void*), &d_out);
  ...
  auto time_launches = [&](std::uint32_t n, std::uint32_t iters, int reps) -> double {
    ...
    ze.zeKernelSetArgumentValue(kernel, 1, sizeof(pc), &pc);
    ...
    for (int i = 0; i < reps; ++i) {
      ...
      ze.zeCommandListAppendLaunchKernel(list, kernel, &groups, sig, 0, nullptr);
  ...
  auto read_back = [&](std::uint32_t count, std::vector<double>& host) {
    ze.zeCommandListReset(list);
    ze.zeCommandListAppendMemoryCopy(list, host.data(), d_out, count * sizeof(double), nullptr, 0, nullptr);
    ze.zeCommandListClose(list);
    ze.zeCommandQueueExecuteCommandLists(queue, 1, &list, nullptr);
    ze.zeCommandQueueSynchronize(queue, UINT64_MAX);
  };
