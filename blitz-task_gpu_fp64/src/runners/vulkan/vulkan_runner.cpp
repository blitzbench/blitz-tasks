 ...
 *
 * Gating: fp64 requires the shaderFloat64 physical-device feature. If the
 * matched device does not report it the row is a clean unsupported(...) result
 * (supported=false), NOT an error. When present it is enabled at device
 * creation so the Float64-capability SPIR-V is legal.
 */
...
RunResult run_gpu_fp64_vulkan(const gpgpu::Setup& setup) {
  ...
  // Enable ONLY shaderFloat64 — the double kernel needs the Float64 capability.
