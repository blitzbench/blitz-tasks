 ...
 *        guarded by the GPU_FP64_HAVE_<BACKEND> defines the linked runner libs export.
 *
 * @param setup
 * @return
 */
...
#if defined(GPU_FP64_HAVE_CUDA)
      return bench::run_gpu_fp64_cuda(setup);
#else
      return missing("CUDA");
#endif
    case gpgpu::BackendId::ROCm:
#if defined(GPU_FP64_HAVE_ROCM)
      return bench::run_gpu_fp64_rocm(setup);
#else
      return missing("ROCm");
#endif
    case gpgpu::BackendId::OneAPI:
#if defined(GPU_FP64_HAVE_ONEAPI)
      return bench::run_gpu_fp64_oneapi(setup);
#else
      return missing("OneAPI");
#endif
    case gpgpu::BackendId::OpenCL:
#if defined(GPU_FP64_HAVE_OPENCL)
      return bench::run_gpu_fp64_opencl(setup);
#else
      return missing("OpenCL");
#endif
    case gpgpu::BackendId::Vulkan:
#if defined(GPU_FP64_HAVE_VULKAN)
      return bench::run_gpu_fp64_vulkan(setup);
#else
      return missing("Vulkan");
#endif
    case gpgpu::BackendId::Metal:
#if defined(GPU_FP64_HAVE_METAL)
      return bench::run_gpu_fp64_metal(setup);
#else
...
GpuFp64::GpuFp64() : timeout_ms_(DEFAULT_BUDGET_MS) {}

GpuFp64::~GpuFp64() = default;

std::string_view GpuFp64::info_json() const noexcept { return GPU_FP64_INFO_JSON; }

blitz::Result GpuFp64::configure(const blitz::DataConfig& cfg) {
...
blitz::Result GpuFp64::set_timeout(const std::uint64_t timeout_ms) {
  timeout_ms_ = timeout_ms;
  return BLITZ_OK;
}

blitz::Result GpuFp64::setSetup(const gpgpu::Setup& setup) {
...
std::vector<bench::gpu::ProbeResult> GpuFp64::probeSetups(
    const std::vector<gpgpu::Setup>& setups) const {
  return bench::gpu::probe_setups(setups, dispatch);
}

blitz::Result GpuFp64::run(const blitz::Callbacks& cb) {
  if (!setup_) {
    if (cb.on_error) {
...
extern "C" ::BlitzTask* gpu_fp64_new(void) {
  return blitz::make_c_task(std::make_unique<gpu_fp64::GpuFp64>());
}
