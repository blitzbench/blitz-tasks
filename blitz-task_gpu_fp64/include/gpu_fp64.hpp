...
/**
 * @class GpuFp64
 * @brief Measures double-precision (fp64) compute throughput on the GPU.
 *
 * runs a dense dependent fused-multiply-add kernel with data kept on-chip, so
 * the result reflects raw fp64 throughput rather than memory transfer. Reported
 * in GFLOPS, higher is better.
 */
class CPP_TASK_DEMO_EXPORT GpuFp64 : public blitz::Task {
 public:
  GpuFp64();
  ~GpuFp64() override;
...
extern "C" {
CPP_TASK_DEMO_EXPORT BlitzTask* gpu_fp64_new(void);
}
