...
/**
 * @class CpuFpMulti
 * @brief All-core floating-point throughput.
 *
 * Every core runs a tight stream of fp32 arithmetic with the working set
 * entirely in registers; the SIMD tier is chosen at runtime (cpu_dispatch.h)
 * and the aggregate is the sum of per-thread rates. Reported in GFLOPS.
 */
class CPP_TASK_DEMO_EXPORT CpuFpMulti : public blitz::Task {
public:
    CpuFpMulti();
    ~CpuFpMulti() override;
...
extern "C" {
    CPP_TASK_DEMO_EXPORT BlitzTask* cpu_fp_multi_new(void);
}
