...
extern "C" const char* CPU_CRYPTO_CHACHA_INFO_JSON;
...
CpuCryptoChacha::CpuCryptoChacha() : timeout_ms_(DEFAULT_BUDGET_MS) {}

CpuCryptoChacha::~CpuCryptoChacha() = default;

std::string_view CpuCryptoChacha::info_json() const noexcept { return CPU_CRYPTO_CHACHA_INFO_JSON; }

blitz::Result CpuCryptoChacha::configure(const blitz::DataConfig& cfg) {
    iterations_ = cfg.iterations;
    return BLITZ_OK;
}

blitz::Result CpuCryptoChacha::set_timeout(std::uint64_t timeout_ms) {
    timeout_ms_ = timeout_ms;
    return BLITZ_OK;
}

blitz::Result CpuCryptoChacha::run(const blitz::Callbacks& cb) {
    ...
    const double bytes_per_sec = bench::run_timed_parallel(
        ...
        [&](unsigned) {
            ...
            return [state, &had_error]() -> std::uint64_t {
                ...
                int len = 0;
                if (!state->ctx ||
                    EVP_EncryptInit_ex(state->ctx, EVP_chacha20_poly1305(), nullptr, kKey, kIv) != 1 ||
    ...
    if (had_error.load()) {
        return fail(BLITZ_ERR_INTERNAL, "EVP ChaCha20-Poly1305 encryption failed");
    }
...
extern "C" ::BlitzTask* cpu_crypto_chacha_new(void) {
    return blitz::make_c_task(std::make_unique<cpu_crypto_chacha::CpuCryptoChacha>());
}
